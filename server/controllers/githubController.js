...
module.exports.getRepositories = async (req, res) => {
  ...
  if (!githubData) {
    return res
      .status(400)
      .json({ message: "User has not installed the GitHub App" });
  }
  try {
    ...
    const simplifiedRepos = repos.data.repositories.map((repo) => ({
      id: repo.id,
      name: repo.name,
    }));

    console.log("GitHub Repositories:", simplifiedRepos);
